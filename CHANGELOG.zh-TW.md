...
### 新增
- 新增涵蓋短碼、目的網址與標題的全帳戶連結搜尋，並提供使用中、已過期、已封存狀態篩選，以及建立時間、更新時間與點擊數排序（#133）。
- 新增依目前搜尋條件維持正確的狀態總數，並以查詢字串保存儀表板清單狀態，支援重新載入、分享與上一頁／下一頁瀏覽。

### 變更
- 將搜尋、篩選、排序、計數與分頁移至管理 API 與 D1 查詢執行，不再只套用於目前頁面已載入的 20 筆連結。
- URL 變更後改以不清空畫面的方式向伺服器重新同步目前清單，正確反映搜尋、狀態、分頁與最近更新排序的變化。
- 釐清儀表板標籤，使未封存 KPI 與使用中狀態列符合其實際統計語意。

### 修正
- 修正符合條件的連結位於其他頁面時，搜尋錯誤顯示無結果，以及點擊數排序無法涵蓋全帳戶的問題。
- 修正無效或超出範圍的頁碼、零結果的狀態計數、非 ASCII 搜尋一致性，以及過期清單回應可能改寫其他路由查詢字串的問題。

### 基礎設施
- 在 migration `0005_add_url_list_indexes.sql` 新增帳戶範圍的 D1 索引，支援 URL 清單排序與啟用狀態查詢。

### 文件
- 更新中英雙語 API、實作摘要與專案結構文件，說明由伺服器處理的 URL 清單契約及 offset 分頁的已知限制。
