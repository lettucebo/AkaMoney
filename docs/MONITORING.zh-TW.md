...
| 重新導向背景回報 | 回報會在全新建立的 current 與 isolation scope 上執行，這些 scope 只帶有所擷取的 client，以及回報本身明確設定的中繼資料（`error` 等級與 `background_operation`、`short_code`、`url_id` 標籤），藉此讓環境中的 request 資料不會進入回報。但這**不是**保證：Sentry 會把 global scope 合併進每一個 event，真正的保證是最終送出的 payload 形狀。`beforeSend` 會辨識 exception 型事件上的 `background_operation=redirect.click_recording` 標籤，並依欄位白名單重建事件（`event_id`、`timestamp`、`platform`、`level`、`environment`、`release`、`dist`、`sdk`、`exception`、`debug_meta`，以及 `background_operation`、`short_code`、`url_id` 三個標籤），因此 request 標頭、`request.cookies`、`user.ip_address`、breadcrumbs、contexts，以及未來 integration 或 global scope 加入的任何欄位都會被丟棄，而不是逐項列黑名單。Sentry log 則以手工組出的 envelope 送出，而非 `Sentry.logger`（後者會在 `beforeSendLog` 之後合併 scope 屬性）；其屬性剛好只有 `operation`、`shortCode`、`urlId`、`errorName`、`sentry.environment`，以及已遮蔽（URL、IPv4／IPv6 位址、憑證指派）且長度受限的 `errorMessage`（`src/redirect/src/sentry.ts:25-42`；`src/redirect/src/sentry.ts:71-114`；`src/redirect/src/sentry.ts:140-148`；`src/redirect/src/sentry.ts:184`；`src/redirect/src/sentry.ts:194-248`；`src/redirect/src/observability.ts:49-136`；`src/redirect/src/observability.ts:206-229`）。 | 目的地 URL、request 標頭、cookies、用戶端 IP 與原始 error 物件都不會附加到這些回報。新增欄位必須同時維持在兩份白名單內：把資料放進 scope 並不能保護 payload，因此移除 `beforeSend` 的重建流程，或改回用 `Sentry.logger` 送 log，都會讓 global／環境資料重新混入。 |
