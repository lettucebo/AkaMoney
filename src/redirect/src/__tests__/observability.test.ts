...
// Keep the key and low-entropy value separate in source so secret scanners do not
// mistake this synthetic redaction fixture for a deployed credential.
const SYNTHETIC_UNDERSCORE_API_KEY = ['x', 'api', 'key'].join('_');
const SYNTHETIC_HEADER_API_KEY = ['X', 'Api', 'Key'].join('-');
const SYNTHETIC_API_KEY_VALUE = ['fixture', 'value', '123'].join('-');
...
describe('background observability primitives', () => {
  ...
  it('redacts credential keys that carry prefixes, suffixes and extra cookie pairs', () => {
    expect(
      toBoundedErrorMessage(
        new Error(`rejected ${SYNTHETIC_UNDERSCORE_API_KEY}=${SYNTHETIC_API_KEY_VALUE}`)
      )
    ).toBe(
      'rejected [redacted]'
    );
    ...
    expect(
      toBoundedErrorMessage(
        new Error(`rejected ${SYNTHETIC_HEADER_API_KEY}: ${SYNTHETIC_API_KEY_VALUE}`)
      )
    ).toBe(
      'rejected [redacted]'
    );
...
describe('credential assignment redaction parity', () => {
  ...
  it('reproduces the documented credential redaction semantics exactly', () => {
    const documented: readonly (readonly [string, string])[] = [
      ...
      [
        `rejected ${SYNTHETIC_UNDERSCORE_API_KEY}=${SYNTHETIC_API_KEY_VALUE}`,
        'rejected [redacted]'
      ],
      ['rejected client_secret=super-secret', 'rejected [redacted]'],
      ['rejected refresh_token=abc123', 'rejected [redacted]'],
      [
        `rejected ${SYNTHETIC_HEADER_API_KEY}: ${SYNTHETIC_API_KEY_VALUE}`,
        'rejected [redacted]'
      ],
